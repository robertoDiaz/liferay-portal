 ...
 * @author Roberto Díaz
 */
public class CoverImageTest extends BaseImageTestCase {
	...
	@Override
	protected long getFileEntryId(BlogsEntry entry) {
		return entry.getCoverImageFileEntryId();
	}
	...
	protected BlogsEntry updateBlogsEntry(
		...
		return BlogsEntryLocalServiceUtil.updateEntry(
			user.getUserId(), entryId, RandomTestUtil.randomString(),
			RandomTestUtil.randomString(), RandomTestUtil.randomString(),
			RandomTestUtil.randomString(), new Date(), true, true,
			new String[0], StringPool.BLANK, coverImageSelector,
			smallImageSelector, serviceContext);
	...
	@Override
	protected BlogsEntry updateBlogsEntry(long entryId, String imageTitle)
		throws Exception {
