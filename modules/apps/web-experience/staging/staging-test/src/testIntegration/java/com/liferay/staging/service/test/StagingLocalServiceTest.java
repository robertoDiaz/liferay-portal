...
public class StagingLocalServiceTest {
	...
	public void testDisableStaging() throws Exception {
		...
		try {
			...
			Group finalGroup = GroupLocalServiceUtil.getGroup(
				group.getGroupId());

			Assert.assertFalse(finalGroup.hasStagingGroup());
		}
		finally {
	...
	public void testEnableLocalStaging() throws Exception {
		...
		try {
			...
			Assert.assertTrue(finalGroup.hasStagingGroup());
		}
		finally {
			GroupLocalServiceUtil.deleteGroup(group.getGroupId());
		}
	...
	public void testEnableLocalStagingWithParentGroup() throws Exception {
		...
		try {
			...
			Group finalGroupStagingGroup = finalGroup.getStagingGroup();

			Assert.assertEquals(
				parentGroup.getGroupId(), finalGroup.getParentGroupId());
			Assert.assertEquals(
				parentGroup.getGroupId(),
				finalGroupStagingGroup.getParentGroupId());
		}
		finally {
			GroupLocalServiceUtil.deleteGroup(group.getGroupId());
	...
	public void testEnableLocalStagingWithStagedChildGroup() throws Exception {
		...
		try {
			...
			Group parentGroupStagingGroup = parentGroup.getStagingGroup();
			Group childGroupStagingGroup = childGroup.getStagingGroup();
