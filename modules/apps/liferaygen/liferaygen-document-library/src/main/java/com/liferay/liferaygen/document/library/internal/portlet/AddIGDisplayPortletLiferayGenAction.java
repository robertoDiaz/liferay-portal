...
@Component(
	immediate = true,
	properties = "liferaygen.action.class.name=com.liferay.liferaygen.document.library.internal.portlet.AddIGDisplayPortletLiferayGenAction",
	service = LiferayGenAction.class
)
public class AddIGDisplayPortletLiferayGenAction
	...
	public String doGetDescription() {
		return StringBundler.concat(
			"Adds a random document library display portlet in a random ",
	...
	public String getName() {
		return "AddIGDisplayPortletLiferayGenAction";
	}
	...
	protected void doUpdatePortletPreferences(
		...
		portletPreferences.setValue(
			"folderColumns",
			_getSelectedEntryColumns("num-of-folders,num-of-documents,action"));

		portletPreferences.setValue(
			"fileEntryColumns",
			_getSelectedEntryColumns("size,downloads,locked,action"));

		portletPreferences.setValue(
			"foldersPerPage",
			String.valueOf(
				liferayGenValueGenerator.getRandomIntegerFromRange(10, 200)));
	...
	private String _getSelectedEntryColumns(String columns) {
		...
		return "name,".concat(StringUtil.merge(selectedEntryColumnsList));
	}
