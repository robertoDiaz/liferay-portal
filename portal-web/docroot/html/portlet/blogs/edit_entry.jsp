...
<aui:form action="<%= editEntryURL %>" enctype="multipart/form-data" method="post" name="fm" onSubmit="event.preventDefault();">
	...
	>
		<liferay-ui:section>
			...
			<%
			boolean customAbstract = (entry == null) ? false : Validator.isNotNull(entry.getDescription());
			%>
			...
			<div id="<portlet:namespace />entry-abstract-options">
				<aui:input checked="<%= !customAbstract %>" cssClass="abstract splitted-content"  label='<%= LanguageUtil.format(request, "use-first-x-characters-of-the-content-entry", 200, false)  %>' name="abstract" type="radio" value="<%= false %>" />
				<aui:input checked="<%= customAbstract %>" cssClass="abstract custom" data-max-length="<%= 200 %>" label="custom-abstract" name="abstract" type="radio" value="<%= true %>" />
			</div>
...
<aui:script use="liferay-blogs">
	...
	Liferay.provide(
		window,
		'<portlet:namespace />OnChangeEditor',
		function(html) {
			blogs.setAbstractContent(html, 200);
		}
	);
